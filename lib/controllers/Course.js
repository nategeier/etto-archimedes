...
var create = function (req, res) {
  ...
  var receivedCourse = req.body;


  async.waterfall([

    function (done) {
      if (receivedCourse.makeCopy === true) {
        Course.getCourse(receivedCourse._id, function (err, results) {
          var course = results.toObject();
          course.title = course.title + " copy";
          done(err, course);
        });
      } else {
        done(null, receivedCourse);
      }
    },
 ...
 *
 */
var setCoursePriority = function (req, res) {


  Course.setCoursePriority(req.params.id, req.params.priority, function (err, metadata) {
...
module.exports = {
  ...
  setCoursePriority: setCoursePriority,
