...
var addBambooUserToTiers = function (user, usersTier, rejectedUsers, invitedAuth, done) {
  ...
  User.getBambooEmployees(apikey, subdomain, function (err, employees) {
    ...
    async.mapSeries(employees, function (employee, done) {
      ...
      async.waterfall([
        ...
        function (results, done) {
          addBambooUserToTiers(user, usersTier, rejectedUsers, invitedAuth, function (err, results) {
            done(err, results);
          });
        }
      ], function (err, results) {
