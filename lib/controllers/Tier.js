      ...
      function (callback) {
        ...
        if (newTier.parent) {
          ...
          async.parallel([
              ...
              function (callback) {
                Tier.findParent(newTier.parent, function (err, results) {
                  if (results && results[0]) {
                    newTier._company = results[0]._company;
                    newTier.ancestors = results[0].ancestors;
              ...
              function (callback) {
                Report.getCourseReportsForTier(newTier.parent, function (err, results) {

                  async.map(results, function (report, callback) {
                      var newReport = new Report({
                        _tier: newTier._id,
                        _course: report._course,
                        _completed: [],
                        _notCompleted: []
                      });
