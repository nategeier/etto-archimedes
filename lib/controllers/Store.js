...
var Subscriptions = require("../../config/seeds/subscriptions");
/**
 * listStoreCourses
 *
 */
 ...
 *
 */
var returnOnlyIds = function (arr, done) {
 ...
 *
 */
var countUsersInCompany = function (companyId, done) {

  async.waterfall([

      //--- Look for stripe customer exists
      function (callback) {
        Tier.listAllCompanyTiers(companyId, function (err, results) {
          callback(err, results);
        });
      },
      function (tiers, callback) {
        returnOnlyIds(tiers, function (tierIds) {
          tierIds.push(companyId);
          callback(null, tierIds);
        });
      },
      function (tierIds, callback) {

        //console.log("lonnngggggg----------", tierIds)
        User.countUsersInCompany(tierIds, function (err, results) {
          callback(err, results);
        });
      }
    ],
...
var checkCanAddCourse = function (req, res) {
  ...
  async.waterfall([
      ...
      function (callback) {
        //---Check if they have the minimum number of employees
        countUsersInCompany(companyId, function (err, empCount) {
          if (Number(empCount) <= Number(Subscriptions[0].empRange.low)) {
            subscribed.isGood = true;
            callback(subscribed);
          } else {
            callback(err, empCount);
          }
        });
      },
      function (empCount, callback) {
        Tier.findTier(companyId, function (err, company) {
          callback(err, empCount, company);
        });
      },
      //--- Look for stripe customer exists
      function (empCount, company, callback) {
        Course.getCeatedCourses(companyId, function (err, results) {
          ...
          if (results.length === 0) {
            ...
            var numCourses = results.length;
            callback(err, empCount, company, numCourses);
          }
        });
      },
      function (empCount, company, numCourses, callback) {
...
var cancelSubscription = function (req, res) {
  ...
  async.waterfall([
      ...
      function (callback) {
        Tier.getCompany(companyId, function (err, company) {
      ...
      function (confirmation, callback) {
        Tier.removeSubscription(companyId, function (err, results) {
          callback(err, confirmation);
        });
      }
    ],
...
module.exports = {
  ...
  countUsersInCompany: countUsersInCompany,
