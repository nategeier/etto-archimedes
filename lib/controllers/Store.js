...
var checkCanAddCourse = function (req, res) {
  ...
  async.waterfall([
      ...
      function(empCount, done){
        Tier.findTier(companyId, function(err, results){
          done(err, empCount, results)
        });
      },
      function (empCount, company, done) {

        console.log(empCount, company);
        Subscription.findSubscription(company._subscription, function (err, results) {
          var subscription = results;

          if (subscription && empCount <= Number(subscription.empRange.high)) {
            subscribed.isGood = true;
          }
          done(err, subscribed);
        });
