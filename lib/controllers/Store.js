...
var findRecipient = function (req, res) {
  ...
  stripe.recipients.retrieve("rp_3XAmisOex6ydlE", function (err, recipient) {
    if (err) {
      return res.json(204);
    }
...
var purchase = function (req, res) {
  ...
  async.waterfall([
      ...
      function (callback) {
        ...
        Credit.updateCredit(order, courseId, subscriptionId, desc, function (err, results) {
          callback(err);
        });
...
var setSubscription = function (subscription, price, customer, done) {
  ...
  async.waterfall([
      ...
      function (callback) {
        ...
        if (customer.subscription.count === 0) {
...
var retrieveAndOrCreateCustomer = function (customerId, card, email, callback) {
  async.waterfall([
      ...
      function (customer, callback) {
        ...
        } else if (customer && !customer.active_card) { //---Customer exists but no active card
