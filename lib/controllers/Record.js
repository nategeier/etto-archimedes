...
var createRecord = function (tier, record, userId, courseId, totalBlocks, done) {
  ...
  Course.getPriceAndCreator(courseId, function (err, course) {
    ...
    if (record || course.price === 0 || !tier) {
      ...
      Record.create(userId, courseId, tierId, progress, function (err, results) {
        done(err, results);
      });
...
var createRecorForUser = function (userId, courseId, done) {
  async.parallel([
    async.apply(User.getUserTier, userId),
    async.apply(Record.userBestCourseRecord, userId, courseId),

  ], function (err, results) {

    done(err, results[0]._tier, results[1]);
  });
};
...
var create = function (req, res) {
  ...
  var userId = req.params.userId || req.query.userId || req.body.userId,
    ...
    totalBlocks = req.params.totalBlocks || req.query.totalBlocks || req.body.totalBlocks;
  ...
  async.waterfall([
      ...
      function (done) {
        if (userId) {
          createRecorForUser(userId, courseId, function (err, tier, bestRecord) {
            done(err, tier, bestRecord);
          });
        } else {
          done(null, null, null);
        }
      ...
      function (tier, record, callback) {
        ...
        createRecord(tier, record, userId, courseId, totalBlocks, function (err, results) {
          callback(err, results);
        });
      }
    ],
...
var updateBookmark = function (req, res) {
  ...
  Record.findOneAndUpdate(query, req.body, function (err, results) {
...
var findCourseRecords = function (req, res) {

  var courseId = req.params.courseId || req.query.courseId || req.body.courseId;


  async.parallel([

    function (done) {
      Record.getCourseComments(courseId, function (err, results) {
        done(err, results);
      });
    },
    function (done) {
      Record.countCourseRecords(courseId, function (err, results) {
        done(err, results);
      });
    },
    function (done) {
      Record.countCourseCompletedRecords(courseId, function (err, results) {
        done(err, results);
      });
    },
    function (done) {
      Course.courseTitle(courseId, function (err, results) {
        done(err, results);
      });
    }
    ...
    var report = {
      ...
      commentedCourses: results[0],
      totalViews: results[1],
      totalPassed: results[2],
      course: results[3]
    };
...
module.exports = {
  ...
  findCourseRecords: findCourseRecords
};
