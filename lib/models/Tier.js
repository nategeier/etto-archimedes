...
Tier.descendants = function(tierID, callback) {
  this.find({ancestors: tierID}, function(err, results){
    callback(err, results);
  });
}

Tier.findParent = function(parentID, callback) {
  this.find({_id: parentID}, function (err, results) {
    callback(err, results);
  });
}

Tier.addChildToParent = function(parentID, tierID, callback) {
  this.update({_id: parentID}, {$addToSet: { _children: tierID }}, function (err, results) {
    callback(err, results);
  }) 
}
...
Tier.listChildrenTiers = function(parentID, callback){
  Tier.find({parent: parentID}, function(err, results){
    callback(err, results);
  });
}
