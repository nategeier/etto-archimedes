...
Record.getCourseComments = function (courseId, done) {
  ...
  Record.find(query, "_course _user comment lastUpdate").populate("_user _course", "comment avatarUrl title thumb name created").exec(function (err, results) {
    done(err, results);
  });
};


Record.countCourseRecords = function (courseId, done) {
...
Record.countCourseCompletedRecords = function (courseId, done) {
  ...
  Record.count(query, function (err, results) {
    done(err, results);
  });
};
